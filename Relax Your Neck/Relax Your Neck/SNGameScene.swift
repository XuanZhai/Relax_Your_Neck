...
class SNGameScene: SKScene, SKPhysicsContactDelegate {
    ...
    weak var gamevc : SceneKitViewController?
    ...
    func BacktoHome(){      // The game is over, going back to home page
        ...
        SceneKitViewController.scoreresult = self.score // Reset the static data
        self.gamevc?.dismiss(animated: false) // Discard the scene
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        for touch in touches {
            ...
            if gameStatus == "notstart" {   // If it is the first touch
                ...
                Character.position = CGPoint(x: Character.position.x, y: SceneKitViewController.charheight)
            }
    ...
    override func update(_ currentTime: TimeInterval) {
        if gameStatus == "start"{       // Update the background while the game is processing
            ...
            // If the nose is in the scene, update the nose.
            if SceneKitViewController.charheight >= 0 && SceneKitViewController.charheight <= self.frame.height {
                Character.position = CGPoint(x: Character.position.x, y:SceneKitViewController.charheight-self.frame.height/2)
            }
            else{
                gamePaused()    // Else pause the game
            }
        }
    }
}
